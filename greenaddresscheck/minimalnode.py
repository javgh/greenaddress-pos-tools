...
START_ERROR_WAIT_TIME = 1
MAX_ERROR_WAIT_TIME = 16
    ...
    def is_valid(self):
        ...
        for tout in self.vout:
            if tout.nValue < 0 or tout.nValue > 21000000L * 100000000L:
                return False
        return True
    def total_outgoing_amount(self):
        amount = 0
        for txout in self.vout:
            amount += txout.nValue
        return amount / COIN_FRAC
    def amount_list(self):
        amounts = []
        for txout in self.vout:
            amounts.append("%s BTC" % utils.format_btc_amount(txout.nValue / COIN_FRAC))
        return " + ".join(amounts)
    def has_multiple_outs(self):
        return len(self.vout) > 1
    def __repr__(self):
...
class MinimalBitcoinNode(asyncore.dispatcher):
    messagemap = {
        "version": msg_version,
        "verack": msg_verack,
        "addr": msg_addr,
    ...
    def __init__(self, dstaddr, dstport):
        ...
        self.error_wait_time = START_ERROR_WAIT_TIME
    ...
    def attempt_connection(self, hadError = False):
        if hadError:
            time.sleep(self.error_wait_time)
            self.error_wait_time *= 2
            if self.error_wait_time > MAX_ERROR_WAIT_TIME:
                self.error_wait_time = MAX_ERROR_WAIT_TIME
    ...
    def got_message(self, message):
        ...
        if message.command  == "version":
            ...
            if message.tx.is_valid():
                self.handle_tx(message.tx)
